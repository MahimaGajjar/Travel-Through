...
from .util import fetchDetails,time_date,fetchData,days,parse, getRoutes, distributeDays,distance,HomeData
...
def getSelectedLocations():
    ...
    while(len(final)>0):
        ...
        #print('soreted dic==',sortedDic)
        currentItem = sortedDic[0]
        l1=[parse(currentItem[1]['latitude']),parse(currentItem[1]['longitude'])]
      
        plan,start,da,ct,f = getRoutes(currentItem,initial,path,day,scheduleList,c,flag,days_num)
